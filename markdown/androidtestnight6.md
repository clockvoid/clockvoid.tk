...
### UIテストで楽をするための技術
* 実装が大変
    →テストケースをDSLでKotlin DSLでEspressoのテストが可能である．
* 実行結果の確認が大変
    →StackTraceだけでは状況が確認できない場合もある
    Composerを使ってテスト結果をわかりやすく！！！（Spoonの後継）
    ログとスクリーンショットを合わせたレポートを提供
* 依存が多くて大変
    * ネットワーク依存をなくす
        OkReplay:OkHTTPの通信をレコードしてReplayしてくれる．
        レスポンスデータの管理を楽にする！！！

### まだまだ大変なことはある
動的なリクエストをどう対応するか？→OkReplayだけでは動的なリクエストを生成できない

CI実行を最適化→PRごとにUIテストをしてしまったらPRごとのジョブの実行時間がクッソ伸びる．Nitrogenとかを使って高速化！
